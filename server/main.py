...
def add_workbook():
    ...
    # Checks to see if the workbook is requested to be public and the user is not approved to do so
    ...
    if form.get('password') is not None:
        ...
        workbook = workbooks(workbook_hash, workbook_data['name'],user.name, password)
    else:
        workbook = workbooks(workbook_hash, workbook_data['name'],user.name, None)
...
def get_workbook():
    ...
    if form.get('password') is not None:
        print(form['author'])
        log_all_users()
        user = users.query.filter_by(author=form['author']).first()
        if user is None:
...
def accounts():
    ...
    if form.get('author') is None:
        ...
        if users.query.filter_by(name=escape(form['name']), password=password).first() is None:
            ...
            # Sends all workbooks that the teacher has created, in order for them to be available even after changing devices
